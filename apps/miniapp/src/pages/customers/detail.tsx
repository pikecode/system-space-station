...
export default function CustomerDetailPage() {
  ...
  const handleSave = async () => {
    if (!form.name.trim()) {
      Taro.showToast({ title: '请填写姓名', icon: 'none' }); return;
    }
    if (!/^1\d{10}$/.test(form.phone)) {
      Taro.showToast({ title: '请填写正确的手机号', icon: 'none' }); return;
    }
      ...
      <ScrollView scrollY style={{ height: '100vh' }}>
        {editing ? (
          <View style={{ paddingBottom: '160rpx' }}>
            ...
            <Picker
              mode='selector'
              range={SOURCE_OPTIONS.map(s => s.label)}
              value={sourceIndex}
            ...
            <View className='field'>
              ...
              <Input className='field__input' placeholder='请输入姓名' maxlength={50}
                value={form.name} onInput={(e) => setForm({ ...form, name: e.detail.value })} />
            </View>
            ...
            <View className='field'>
              ...
              <Input className='field__input' type='number' placeholder='请输入手机号' maxlength={11}
                value={form.phone} onInput={(e) => setForm({ ...form, phone: e.detail.value })} />
            </View>
            ...
            <View className='field'>
              ...
              <Input className='field__input' placeholder='选填' maxlength={64}
                value={form.wechat} onInput={(e) => setForm({ ...form, wechat: e.detail.value })} />
            </View>
            ...
            <View className='field'>
              ...
              <Input className='field__input' placeholder='多个标签用逗号分隔'
                value={form.tags} onInput={(e) => setForm({ ...form, tags: e.detail.value })} />
            </View>
